...
test('new supplied media, stable chapters, empty pauses, forward and reverse seeking', async ({page},info) => {
  ...
  await expect(video).toHaveAttribute('src',info.project.name.includes('mobile')?/v7\/mobile.mp4/:/v7\/desktop.mp4/);
...
test('desktop wheel pause visits every visual stop and aligns text',async({page},info)=>{
  ...
  expect(await page.locator('[data-chapter="2"]').evaluate(el=>Number(getComputedStyle(el).opacity))).toBeGreaterThan(.98);
  await page.mouse.wheel(0,180);
  await page.waitForTimeout(3700);
...
test('reduced motion and video failure retain a short readable poster',async({page},info)=>{
  ...
  await page.route('**/media/lemark/v7/desktop.mp4',route=>route.abort());
  await ready(page);
  await expect(page.locator('.lm-story')).toHaveClass(/lm-story--static/);
