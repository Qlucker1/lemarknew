...
if (!/^v\d+$/.test(version)) throw new Error('Expected media version such as v3');
const root = `public/media/lemark/${version}`;
...
for (const name of ['source.mp4','desktop.mp4','mobile.mp4','poster-desktop.jpg','poster-mobile.jpg','contact-sheet.jpg']) {
  ...
  manifest.outputs.push({ path:`/media/lemark/${version}/${name}`, bytes:(await stat(`${root}/${name}`)).size, sha256:createHash('sha256').update(bytes).digest('hex'), ...(jobs.find(j => j.name === name) ? { ffmpegArgs:jobs.find(j => j.name === name).args } : {}) });
}
